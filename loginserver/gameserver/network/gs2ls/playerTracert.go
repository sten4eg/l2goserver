...
func PlayerTracert(data []byte, db database.Database) {
...
func setAccountLastTracert(account string, pcIp string, hop1 string, hop2 string, hop3 string, hop4 string, db database.Database) error {
	_, err := db.Exec(context.Background(), AccountIpsUpdate, pcIp, hop1, hop2, hop3, hop4, account)
	if err != nil {
		return err
	}
	return nil
